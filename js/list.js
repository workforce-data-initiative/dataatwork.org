...
if (typeof BUCKET_LINK_NAME == 'undefined') {
  var BUCKET_LINK_NAME = BUCKET_URL;
}
...
function buildNavigation(info) {
  var root = '<a href="?prefix=">' + BUCKET_LINK_NAME + '</a> / ';
  if (info.prefix) {
  ...
  var files = $.map(xml.find('Contents'), function(item) {
    ...
    return {
      Key: item.find('Key').text(),
          LastModified: item.find('LastModified').text().slice(0, -5),
